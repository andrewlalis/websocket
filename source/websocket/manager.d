 ...
 * and passes them off to their connection's message handler, all in a non-
 * blocking fashion, on a single thread.
 *
 * This manager is suitable for use both on a server or a client, and can be
 * integrated with just about any implementation, as long as you can get the
 * underlying `Socket`.
 *
 * If you're running a server and you notice performance becoming an issue, it
 * is easy to simply spawn more manager threads, and distribute incoming
 * connections among them round-robin style.
 */
class WebSocketManager : Thread {
    /// The internal set of active connections managed by this manager.
    private WebSocketConnection[UUID] connections;

    /// A mutex to control access to the tracked set of connections.
    private ReadWriteMutex connectionsMutex;

    /// The socket set used for non-blocking socket reading.
    private SocketSet readableSocketSet;

    /// An id-indexed set of continuation frames for large websocket messages.
    private WebSocketFrame[UUID] continuationFrames;

    /// Internal flag used to control the main event loop.
    ...
    void registerConnection(Socket socket, WebSocketMessageHandler handler) {
        socket.blocking(false); // Set the socket as non-blocking, because this is a non-blocking manager.
